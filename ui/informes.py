...
def abrir_csv(csv):
    ...
    if sys.platform != 'win32':     # Más general que os.name (que da "nt" en los windows 2000 de las oficinas).
        if not ( (not os.system('oocalc2 "%s" || oocalc "%s" &' % (csv, csv))) or \
                 (not os.system('oocalc "%s" &' % csv)) ):
...
def mandar_a_imprimir_con_acrobat(fichero):
    """
    ...
    NO USAR CON ROLLOS: No cuadra bien la etiqueta y además deja abierta la ventana después.
    Impresora CAB harcoded (y además no es el nombre por defecto de la impresora).
    """
    ...
    comando = """gswin32c.exe -dQueryUser=3 -dNoCancel -dNOPAUSE -dBATCH -sDEVICE=mswinpr2 -sOutputFile="%%printer%%CAB" %s """ % (fichero)
    # NOTA: Necesita que: 
    # 1.- La impresora CAB esté como predeterminada en la "carpeta" impresoras de Windows.
    # 2.- Tenga la configuración adecuada por defecto (apaisado, tamaño de etiqueta, etc.
    # 3.- gs esté en el PATH (añadiendo C:\Archivos de programa...\bin en la variable de entorno PATH desde las propiedades avanzadas de Mi PC.)
...
def imprimir_con_gs(fichero, impresora = None, blanco_y_negro = False):
    ...
    Si impresora es None, imprime sin intervención del usuario en la impresora por defecto.
    ...
    if os.name == "posix":
        ...
        if ruta_a_gs == None:
            ...
            if salida != 0 and salida != 1:     # gs devuelve 1 si le da a Cancelar.
                print "informes.py (imprimir_con_gs): No se pudo imprimir. Lanzo el visor."
                abrir_pdf(fichero)
            if salida == 1:     # Si cancela la impresión a lo mejor quiere verlo en pantalla.
                abrir_pdf(fichero)
