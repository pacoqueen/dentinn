...
# PLAN: Hay que migrar de mx a datetime...
try:
...
def parse_porcentaje(strfloat, fraccion = False):
    ...
    corresponde con el valor del porcentaje, es decir, 
    en la forma 0. (10 % = 0.1).
    Si fraccion es True devuelve el valor como fracción de 1.
    """
...
def mostrar_calendario(fecha_defecto = time.localtime()[:3][::-1], 
    ...
    elif isinstance(fecha_defecto, datetime.date):
...
def es_interpretable_como_fechahora(f):
    ...
    """
    if convertir_a_fechahora(f) != None:
        res = True
    else:
        res = False
    return res
...
def parse_fechahora(txt):
    ...
    return fecha + hora     # Curioso... la suma de DateTime y DateTimeDelta no tiene la propiedad conmutativa.
...
def float2str(n, precision = 2, autodec = False):
    """
    ...
    if precision == 0:
        ...
        if isinstance(n, (float, Ffloat, int, long)):
            if n < 0:   # Es negativo
                negativo = True
                n = -n  # Lo hago positivo y después le añadiré el signo -.
            else:
...
def cmp_fecha_id(obj1, obj2):
    ...
    if obj1 == None and obj2 != None:
        return 1
    if hasattr(obj1, "fecha") and obj1.fecha != None and hasattr(obj2, "fecha") and obj2.fecha != None:
        if obj1.fecha < obj2.fecha:
            return -1
        if obj1.fecha > obj2.fecha:
            return 1
        return 0
    if hasattr(obj1, "fecha") and obj1.fecha != None and (not hasattr(obj2, "fecha") or obj2.fecha == None):
        return -1
    if (not hasattr(obj1, "fecha") or obj1.fecha == None) and hasattr(obj2, "fecha") and obj2.fecha != None:
        return 1
...
def pedir_producto_compra(padre = None, proveedor = None):
    ...
    proveedor -> Si es != None, sólo buscará entre los productos de ese proveedor.
    """
    import sys, os
    ...
    if codigo_o_descripcion != None:
        ...
        if proveedor == None:
            ...
            productos = [p for p in productos if proveedor in p.proveedores]
        if len(productos) > 1:
            filas = [(p.id, p.codigo, p.descripcion, ", ".join([pro.nombre for pro in p.proveedores])) for p in productos]
            ...
            if proveedor != None:
                dialogo_info(titulo = "PRODUCTO NO ENCONTRADO", 
                             texto = "No se encontró el producto o bien el mismo no    \nha sido servido nunca por %s." % (proveedor.nombre), 
                             padre = padre)
            else:
                dialogo_info(titulo = "PRODUCTO NO ENCONTRADO", 
...
def orden_por_fecha_entrega_o_id(ldp1, ldp2):
    ...
    """
    if ldp1.fechaEntrega != None and ldp2.fechaEntrega != None:
        if ldp1.fechaEntrega < ldp2.fechaEntrega:
            return -1
        if ldp1.fechaEntrega > ldp2.fechaEntrega:
            return 1
        return 0
    elif ldp1.fechaEntrega != None and ldp2.fechaEntrega == None:
        return -1
    elif ldp1.fechaEntrega == None and ldp2.fechaEntrega != None:
        return 1
    elif ldp1.fechaEntrega == None and ldp2.fechaEntrega == None:
...
def abs_mxfecha(fecha):
    ...
    """
    return mx.DateTime.DateTimeFrom(day = fecha.day, month = fecha.month, year = fecha.year) # hour, minutes... por defecto son 0
...
def buscar_productos_compra(a_buscar):
    """
                    ...
                    if cambiar_orden_pvp(tarifa):
                        if incluir_sin_iva:
                            ...
                            resultados[-1].insert(4, "%s" % (
                                float2str(preciotarifa * 1.16)))
                        else:
                            resultados[-1].insert(3, "%s (%s %%)" % (float2str(preciotarifa * 1.16), 
                                                  float2str(tarifa.get_porcentaje(pc, precio_cache = preciotarifa), 1))
                                                 )
                    else:
                        if incluir_sin_iva:
                            resultados[-1].append("%s (%s %%)" % (float2str(preciotarifa), 
                                                  float2str(tarifa.get_porcentaje(pc, precio_cache = preciotarifa), 1)))
                            resultados[-1].append("%s" % (float2str(preciotarifa * 1.16)))
                        else:
                            resultados[-1].append("%s (%s %%)" % (float2str(preciotarifa * 1.16), 
                                                  float2str(tarifa.get_porcentaje(pc, precio_cache = preciotarifa), 1))
                                                 )
                ...
                for tarifa in tarifas:
                    ...
                    if cambiar_orden_pvp(tarifa):
                        if incluir_sin_iva:
                            ...
                            resultados[-1].insert(4, "%s" % (float2str(preciotarifa * 1.16)))
                        else:
                            resultados[-1].insert(3, "%s (%s %%)" % (float2str(preciotarifa * 1.16), 
                                                  float2str(tarifa.get_porcentaje(pv, precio_cache = preciotarifa), 1))
                                                 )
...
def seleccionar_repuesto(tipo, ventana_padre = None, dejar_crear = True):
    ...
    if not dejar_crear:
        if prods.count() == 0:
            dialogo_info(titulo = "SIN DATOS", 
                         texto = "No hay repuestos dados de alta con el tipo de material «%s»." % pclases.TipoDeMaterial.get(idtipo).descripcion, 
                         padre = ventana_padre)
        else:
            ...
            idprod = dialogo_combo(titulo = "SELECCIONE PRODUCTO", 
                                   ...
                                   ops = prods)
            if idprod != None:
                res = pclases.ProductoVenta.get(idprod)
    else:
...
def cambiar_por_combo(tv, numcol, opts, clase, campo, ventana_padre = None):
    ...
    """
    from pclases import getObjetoPUID
